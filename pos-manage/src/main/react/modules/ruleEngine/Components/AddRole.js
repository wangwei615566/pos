...
var AddRole = React.createClass({
  ...
  handleOk(e) {
   ...
   // //console.log("tableInfo44444444444",tableInfo);
    this.props.form.validateFields((errors, values) => {
      ...
      params.typeResultStatus=data.typeResultStatus;
      ...
      if (this.props.title == '编辑') {
        url = '/modules/manage/rule/saveRuleConfig.htm'; //+ '?name=' + params.name + '&id=' + this.props.ruleData.rule.id + '&configList=' + JSON.stringify(params.configList);
         paramsData={"name": params.name,"id":this.props.ruleData.rule.id,"configList": JSON.stringify(params.configList),"infoList":JSON.stringify(params.infoList),"type":params.type,"typeResultStatus":params.typeResultStatus}
       // //console.log(url)
      } else {
       // //console.log(params.name)
        url = '/modules/manage/rule/saveRuleConfig.htm';  // + '?name=' + params.name + '&configList=' + JSON.stringify(params.configList);
         paramsData={"name":params.name,"configList":JSON.stringify(params.configList),"infoList":JSON.stringify(params.infoList),"type":params.type,"typeResultStatus":params.typeResultStatus}
      };
      Utils.ajaxData({
        url: url,
        data:paramsData,
  ...
  onChange(e){
    ...
    this.setState({
       typeResultStatus:e.target.value,
       itemTwo:true
    })
  },
 ...
 //   //console.log("state.keys2",state.keys2);
              ...
              <Col span="8" className='col_h'>
               ...
               <Col span="6" className="col_h">
                <FormItem  {...{ labelCol: {span: 6},wrapperCol: {span: 15}}} label="分值:">
                 <InputNumber   type="number"    {...getFieldProps(`${k}info_integral`,{initialValue:this.formData2?this.formData2[`${k}integral`]:''})} />
                </FormItem>
               </Col>
             <Col span="6" className="col_h">
         ...
         <Input type='hidden' {...getFieldProps(`${k}id`,{initialValue:this.formData?this.formData[`${k}id`]:0})}/>
          ...
          <Col span="4" className='col_h'>
            <FormItem  {...{ labelCol: {span: 6},wrapperCol: {span: 15}}}
              label="值:">
              <Input  {...getFieldProps(`${k}cvalue`,{initialValue:this.formData?this.formData[`${k}cvalue`]:''})} type="text" autoComplete="off" />
            </FormItem>
          </Col>
               {this.state.type=='10'?
                <Col span="4" className="col_h">
                <FormItem  {...{ labelCol: {span: 6},wrapperCol: {span: 15}}} label="分数:">
                 <InputNumber   type="number"  {...getFieldProps(`${k}integral`,{initialValue:this.formData?this.formData[`${k}integral`]:''})}/>
                </FormItem>
             </Col>
             :<Col span="4" className="col_h">
                    <FormItem  {...{ labelCol: {},wrapperCol: {span:20}} }
            ...
            <Row>
             <Col span='6' className='col_h'>
                <FormItem  {...{ labelCol: {span: 8},wrapperCol: {span: 16}}}        
                  label="规则名称：">
                  <Input  {...getFieldProps('name', {initialValue:"", rules: [{required:true,message: '请输入规则名称，最多20个字!'}]})} type="text" autoComplete="off" />
                </FormItem>
              </Col>
             <Col span='6' className='col_h'>
                <FormItem  {...{ labelCol: {span:6},wrapperCol: {span: 14}}}
                  label="模式：">
                    <Select style={{ width: '100%' }} {...getFieldProps('type', {initialValue: '10',onChange:this.change})} >
              ...
              {this.state.type=="10"?(
                <Col span="6">
                    <FormItem  {...{ labelCol: {span:9},wrapperCol: {span:15}} }
                      label="是否启用决策引擎:">
                     <RadioGroup  {...getFieldProps("typeResultStatus",{initialValue:"",onChange:this.onChange})}>
      ...
      {/*----------------------------------------------------------------------------------------------------------------------------------------------------*/}

             {
              ...
              <div>
               ...
               <Col span="6" className="col_h">
                <FormItem  {...{ labelCol: {span: 6},wrapperCol: {span: 15}}} label="分数:" >
                 <InputNumber   type="number"   {...getFieldProps('0info_integral',{initialValue:this.formData2?this.formData2["0integral"]:''})} />
                </FormItem>
               </Col>
             <Col span="6" className="col_h">
                    <FormItem  {...{ labelCol: {},wrapperCol: {span:20}} }
    ...
    {this.state.keys2.length && this.state.type=="10" && this.state.typeResultStatus=="20"? formItem2 :''}
  {/*-----------------------------------------------------------------------------------------------------------------------------------------*/}
           ...
           <Input type='hidden' {...getFieldProps('0id',{initialValue:this.formData?this.formData['0id']:0})}/>
             <Col span="5" className='col_h'>
              ...
              <Col span="4" className='col_h'>
                <FormItem  {...{ labelCol: {span: 6},wrapperCol: {span: 15}}}
                  label="值:">
                  <Input  {...getFieldProps('0cvalue',{initialValue:this.formData?this.formData["0cvalue"]:''})} type="text" autoComplete="off" />
                </FormItem>
              </Col>
           {this.state.type=='10'?
                <Col span="4" className="col_h">
                <FormItem  {...{ labelCol: {span: 6},wrapperCol: {span: 15}}} label="分数:">
                 <InputNumber   type="number"  {...getFieldProps('0integral',{initialValue:this.formData?this.formData["0integral"]:''})} />
                </FormItem>
             </Col>
             :<Col span="4" className="col_h">
                    <FormItem  {...{ labelCol: {},wrapperCol: {span:20}} }
