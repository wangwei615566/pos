...
let SeachForm = React.createClass({
  ...
  handleQuery() {
    ...
    this.props.passParams({
      searchParams : encodeURIComponent(JSON.stringify(params)),
      pageSize: 10,
      current: 1,
    });
  },
  ...
  handleOut() {
        ...
        window.open("/modules/manage/urgeRepayOrder/export.htm?searchParams="+encodeURIComponent(json));
    ...
    return (
      <Form inline >
        <FormItem label="借款人姓名：">
          <Input onPressEnter={this.handleQuery}  {...getFieldProps('borrowName') } />
        </FormItem>
        <FormItem label="手机号码：">
          <Input onPressEnter={this.handleQuery} {...getFieldProps('phone') } />
        </FormItem>
        <FormItem label="催收人姓名：">
        <Input onPressEnter={this.handleQuery} {...getFieldProps('userName') } />
        </FormItem>
        <FormItem label="逾期天数：">
           <Input onPressEnter={this.handleQuery}  {...getFieldProps('penaltyDayScope') } />
        </FormItem>
        <FormItem label="催收状态:">
