...
let SeachForm = React.createClass({
    getInitialState() {
     ...
     handleQuery() {
        var params = this.props.form.getFieldsValue();
         var json = {startDate:'',endDate:'',name:params.name};
         if(params.time){
     ...
     if(params.channelId){
    	 json.channelId = params.channelId
     }
    ...
    render() {
        ...
        return (
            <Form inline>
             <FormItem label="日期:">
                  <RangePicker disabledDate={this.disabledDate} {...getFieldProps('time', { initialValue: [year+'-'+month+'-'+day,year1+'-'+month1+'-'+day1] }) } />
             </FormItem>
             <FormItem label="渠道:">
