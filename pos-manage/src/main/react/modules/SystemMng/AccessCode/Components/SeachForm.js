  ...
  handleQuery() {
    ...
    var json = {startTime:'',endTime:'',userName:'',name:'',state:''};
     ...
     if(params.time){
        ...
        json.endTime=d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate();
        var d1 = new Date(params.time[0]);  
        json.startTime=d1.getFullYear() + '-' + (d1.getMonth() + 1) + '-' + d1.getDate();
        //console.log(d);
      }
  ...
  render() {
    ...
    return (
      <Form inline >
        ...
        <FormItem label="用户名:">
          <Input onPressEnter={this.handleQuery} {...getFieldProps('userName', { initialValue: '' }) } />
        </FormItem>
        <FormItem label="用户姓名:">
