...
export default React.createClass({
    ...
    passVerifyBorrow(title, record) {
        var record = record;
        confirm({
            title: "是否确定通过机审",
            onOk: function () {
                Utils.ajaxData({
                    url: '/modules/manage/borrow/passVerifyBorrow.htm',
                    method: "post",
                    data: {
                        borrowId:record.id,state:'18'
                    },
                    callback:(result) => {
                        if(result.code == 200){
                            Modal.success({
    ...
    render() {
        ...
        var columns = [{
        	...
        	title: '拒单原因',
            render(text, record) {
        		var info = record.reason;        		
        		if(info && info.length>=8){
        			info = info.substr(0,7)+"...";
        		}     		
                return (
                        <div title={record.reason} style={{ textAlign: "left" }}>
                        	{info}                        	
                        </div>
               )        		
            }
            ...
            render(text, record) {
                if (record.state == '22') {
                    ...
                    return (
                        <div style={{ textAlign: "left" }}>
                        	<a href="#" onClick={me.passVerifyBorrow.bind(me, '通过机审', record, false)}>通过机审</a>&nbsp;&nbsp;&nbsp;
        ...
        return (
            <div className="block-panel">
                ...
                />
                <Lookdetails ref="Lookdetails" key={state.keyModal} dataForm={state.dataForm} recordSoure={state.recordSoure} dataRecord={state.dataRecord} visible={state.visible} title={state.title} hideModal={me.hideModal} record={state.rowRecord}
                    canEdit={state.canEdit} riskRecord={state.riskRecord}/>
            </div>
        );
    }
})
