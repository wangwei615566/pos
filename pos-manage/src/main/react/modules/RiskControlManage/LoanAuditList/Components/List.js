        ...
        Utils.ajaxData({
            ...
            callback: (result) => {
                ...
                this.setState({
                    loading: false,
                    data: result.data,
                    countNoAmount:result.countNoAmount,
                    pagination
                });
            }
        });
...
//        	record.state1 = title == "人工复审" ? "26" : record.state;
                ...
                if (record.state == '22') {
                    return (
                        <div style={{ textAlign: "left" }}>
                            <a href="#" onClick={me.showModal.bind(me, '放款审核', record, true)}>放款审核</a>
                        </div>
                    )
                } 
                else {
        ...
        return (
            <div className="block-panel">
	            <div hidden={true} className="actionBtns" style={{ marginBottom: 16 }}>
		            <button onClick={me.addModal.bind(me,'批量')} disabled={!hasSelected} className="ant-btn">
		                批量放款
		            </button>
		        </div>
                ...
                />
                总金额：{this.state.countNoAmount}
                <AddWin ref="AddWin" key={state.keyModal} dataForm={state.dataForm} recordSoure={state.recordSoure} dataRecord={state.dataRecord} visible={state.visible} title={state.title} hideModal={me.hideModal} record={state.rowRecord}
