    ...
    return (
      <Modal title={props.title} visible={props.visible} onCancel={this.handleCancel} width="500" footer={modalBtns} maskClosable={false} >
        <Form horizontal form={this.props.form}>
          ...
          <Row>
            <Col span="20">
              <FormItem  {...formItemLayout} label="点击选择文件:">
                <Input id="file" style={{display:'none'}} {...getFieldProps('file',{onChange:this.handleFileChange,initialValue:'' })} type="file" />
                <span title={state.fileName?state.fileName:''} id="file1">{state.fileName?state.fileName.substring(0,20)+'...':''}</span>
              </FormItem>
            </Col>
          </Row>
        </Form>
