...
export default React.createClass({
    ...
    fetch(params = {}) {
        ...
        if (!params.pageSize) {
            ...
            params = {
                pageSize: 10,
                current: 1,
                searchParams: JSON.stringify({startDate:year+'-'+month+'-'+day+' 00:00:00',endDate:year1+'-'+month1+'-'+day1+' 23:59:59',state:"50"}),
            }
        }
        if(!params.searchParams){
            params.searchParams= JSON.stringify({startDate:year+'-'+month+'-'+day+' 00:00:00',endDate:year1+'-'+month1+'-'+day1+' 23:59:59',state:"50"});
        }
        Utils.ajaxData({
    ...
    arbitration(title, record) {
        confirm({
            title: "确定提交案件？",
            onOk: function () {
		        Utils.ajaxData({
		            url: "/modules/manage/borrow/submitArbitrate.htm",
		            method: "post",
		            data: {
    ...
    filing(title, record) {
            confirm({
                ...
                onOk: function () {
                    Utils.ajaxData({
                        url: '/modules/manage/borrow/applyArbitrate.htm',
                        method: "post",
                        data: {
                      	  loanBillNos: record.orderNo,
                        },
            ...
            render: (value, record) => {
                return (
                    <div style={{ textAlign: "left" }}>
                        ...
                        {record.arbState=="4"||record.arbState=="13"||record.arbState==undefined?<a href="#" onClick={me.arbitration.bind(me, '提交案件', record, false)}>提交案件</a>:(record.arbState=="0"?<a href="#" onClick={me.filing.bind(me, '申请立案', record, false)}>申请立案</a>:"")}
                    </div>
                )
            }
        }];
