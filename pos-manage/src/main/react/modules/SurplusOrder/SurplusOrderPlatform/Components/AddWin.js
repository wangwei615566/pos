...
var AddWin = React.createClass({
    ...
    handleOk(e) {
        ...
        //console.log(record)
        var url = "/modules/manage/surplusOrderPlatform/save.htm";
        this.props.form.validateFields((errors, values) => {
            ...
            Utils.ajaxData({
                url: "/modules/manage/surplusOrderPlatform/save.htm",
                method: 'post',
                status:data.status,
                data: {
                	name: values.name,
                    creatId: values.creatId,
        ...
        return (
            <Modal title={props.title} visible={props.visible} onCancel={this.handleCancel} width="700" footer={modalBtns} maskClosable={false} >
                <Form horizontal form={this.props.form}>
                    ...
                    <Row>
                        <Col span="16">
                            <FormItem  {...formItemLayout} label="渠道网址：">
                                <Input disabled={!props.canEdit}  {...getFieldProps('channelUrl', { rules: [{ required: true, message: '必填' }] }) } type="text"/>
                            </FormItem>
                        </Col>
                    </Row>
                    <Row>
                    ...
                    <Row>
                    	<Col span="16">
	                        <FormItem  {...formItemLayout} label="平台介绍：">
	                        <Input disabled={!props.canEdit}  {...getFieldProps('introduce', { rules: [{ required: true, message: '必填' }] }) } type="text" />
	                        </FormItem>
                        </Col>
                    </Row>
                </Form>
