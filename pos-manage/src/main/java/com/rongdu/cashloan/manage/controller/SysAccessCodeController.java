...
public class SysAccessCodeController extends BaseController {
	...
	@RequiresPermission(code = "modules:manage:accessCode:add",name = "新增访问码或修改访问码")
	public void add(@RequestParam(value="id") Long id,
		...
		Map<String,Object> result = new HashMap<String,Object>();
		SysAccessCode accessC = sysAccessCodeService.getById(id);
		...
		if ("create".equalsIgnoreCase(status)) {
			...
			if(codeCount > 0){
				result.put(Constant.RESPONSE_CODE, Constant.FAIL_CODE_VALUE);
				result.put(Constant.RESPONSE_CODE_MSG, "修改失败，该用户已存在此访问码，请重新输入访问码");
			} else {
				SysAccessCode accessCode = new SysAccessCode();
				accessCode.setId(id);
				accessCode.setCode(code);
				accessCode.setCreateTime(accessC.getExceedTime());
	...
	public void listUserName(){
	  ...
	  * @throws Exception
	  */
	 @RequestMapping(value = "/modules/manage/user/accessCode/delete.htm" ,method={RequestMethod.GET,RequestMethod.POST})
//     @RequiresPermission(code = "modules:manage:system:accessCode:delete", name = "访问码删除")
	 public void accessCodeDelete(@RequestParam(value="id") Long id,
							HttpServletRequest request, HttpServletResponse response) throws Exception {
		 Map<String, Object> result = new HashMap<String, Object>();
		 int msg = sysAccessCodeService.deleteById(id);
		 if (msg > 0) {
			 result.put(Constant.RESPONSE_CODE, Constant.SUCCEED_CODE_VALUE);
