...
import com.czwx.baofoo.service.BaofooPayService;
...
@Component
@Lazy(value = false)
public class QuartzBfRepayment implements Job {

	private static final Logger logger = Logger.getLogger(QuartzBfRepayment.class);
		...
		try {
			for (BorrowRepay borrowRepay : borrowRepayList) {
				...
				if (null != repaymentLog
					...
					params.put("orig_trans_id", repaymentLog.getOrderNo());//商户订单号
					params.put("trans_serial_no", payReqTime);//商户流水号
					params.put("trade_date", payReqTime);
					Map<String,String> res = BaofooPayService.query(params);
					// 订单支付成功状态
				...
				//支付请求
				Map<String,String> params = new HashMap<String,String>();
				params.put("pay_code", bankCard.getBankCode());
				params.put("acc_no", bankCard.getCardNo());
				params.put("id_card", ubi.getIdNo());
				params.put("id_holder", ubi.getRealName());
				params.put("mobile", bankCard.getPhone());
				params.put("txn_amt", String.valueOf(amount));
				params.put("trans_id", orderNo);//商户订单号
				params.put("trans_serial_no", payReqTime);//商户流水号
				params.put("trade_date", payReqTime);
				
	            PayReqLog payReqLog = new PayReqLog();
				payReqLog.setOrderNo(orderNo);
				payReqLog.setService("宝付自动代扣");
				payReqLog.setCreateTime(DateUtil.getNow());
				payReqLog.setParams(JSON.toJSONString(params));
	...
	public void execute(JobExecutionContext context)
		...
		// 查询当前任务信息
		QuartzInfo quartzInfo = quartzInfoService.findByCode("doRepayment2");
